...
export const patientRoute = createTRPCRouter({
  getPatient: publicProcedure.input(z.object({barcode: z.string(), locationId: z.number(), studentId: z.string() || z.null()})).mutation(async ({input, ctx})=>getPatientByBarCode(ctx.db, input.barcode, input.locationId, input.studentId)),
