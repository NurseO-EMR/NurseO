...
import {
    ...
    limit, query, updateDoc, where, setDoc, doc, getDoc, orderBy, deleteDoc, DocumentData, QueryDocumentSnapshot, Firestore
} from "firebase/firestore";
import { $error, $patient, $settings } from "./State.js";
import { PatientChart } from "../Types/PatientProfile.js";
import { PatientNotFoundError } from "../Types/ErrorCodes.js";
...
export class Database {
    ...
    async getMedication(medID?: string, barcode?: string): Promise<Medication | null> {
        ...
        if (medID) {
            doc = <QueryDocumentSnapshot<DocumentData>> await this.getMedicationDoc(medID);
        } else if (barcode) {
            doc = <QueryDocumentSnapshot<DocumentData>> await this.getMedicationDoc(undefined, barcode);
        } else {
