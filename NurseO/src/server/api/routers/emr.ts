...
export const EMRRouter = createTRPCRouter({
    student_getPatient: publicProcedure.input(z.object({ barcode: z.string(), locationId: z.number(), studentId: z.string() || z.null() })).mutation(async ({ input, ctx }) => await getPatientByBarCode(ctx, input.barcode, input.locationId, input.studentId)),
